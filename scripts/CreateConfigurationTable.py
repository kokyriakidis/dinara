...
"""
This must be run from the dinara directory.
It expects to see a conf and src directory.

This creates dinara/src/ConfigurationTable.cpp which contains
the definition of dinara::configurationTable,
a map that map configuration names to strings.

The file will be overwritten if it exists.

Each entry in the table corresponds to a file in dinara/conf.

"""

# Check that we have the conf and src directories.
if not (os.path.isdir('conf') and os.path.isdir('src')):
    raise Exception('Must run from the dinara directory')
