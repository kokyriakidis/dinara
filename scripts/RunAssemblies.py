...
It assumes that the Dinara executable is in the
same directory that contains this script
(typically, that will be the dinara-install/bin directory).
...
# Locate the Dinara executable. It is assumed to be in the
# same directory that contains this script.
dinaraExecutable = os.path.dirname(os.path.realpath(__file__)) + '/dinara'
if not os.path.exists(dinaraExecutable):
    raise Exception('Dinara executable not found at %s' % dinaraExecutable)
