...
# Iterate the dinara CSV and yield one PhasedRead data object at a time
def parseDinaraPhaseCsv(dinaraPath):
    with open(dinaraPath, 'r') as file:
...
def main(hap1MarginPath, hap2MarginPath, dinaraPath):
    discordantCsvPath = os.path.join(os.path.dirname(dinaraPath), "DiscordantPhasedReads.csv")

    marginPhases = (set(),set())
    dinaraComponents = defaultdict(lambda: (set(),set()))
    nameToId = dict()

    # Read all the Dinara phase assignments
    for read in parseDinaraPhaseCsv(dinaraPath):
        dinaraComponents[read.component][read.phase].add(read.name)
        nameToId[read.name] = read.id

        if read.name in dinaraComponents[read.component][1-read.phase]:
            exit("ERROR: read contained both in hap1 and hap2 of Dinara phases: " + read.name)
    ...
    # Compare Dinara to Margin and write results to CSV + stdout
    with open(discordantCsvPath, 'w') as file:
        file.write("OrientedReadId,ReadName,Component,DinaraPhase,MarginPhase\n")

        for componentId,component in dinaraComponents.items():
            print("Evaluating component:", componentId)

            names = list()
            dinaraPhaseArray = list()
            marginPhaseArray = list()

            # Compare phase assignments for Dinara and the user-provided dataset, for each read in each component
            ...
            for i in range(len(names)):
                countMatrix[0][dinaraPhaseArray[i]] += 1
                countMatrix[1][marginPhaseArray[i]] += 1

                confusionMatrix[dinaraPhaseArray[i]][marginPhaseArray[i]] += 1
            ...
            # Indicate which coordinates in the confusion matrix represent discordant reads between dinara's
            ...
            for i in range(len(names)):
                coordinate = (dinaraPhaseArray[i], marginPhaseArray[i])
                ...
                if coordinate == discoordinateA or coordinate == discoordinateB:
                    ...
                    file.write(',')
                    file.write(str(dinaraPhaseArray[i]))
                    file.write(',')
                    file.write(str(marginPhaseArray[i]))
                    file.write('\n')

            print("Dinara phase counts:",countMatrix[0])
            ...
            adjustedRandIndex = metrics.adjusted_rand_score(marginPhaseArray, dinaraPhaseArray)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path of Dinara phase components csv (Phasing.csv)"
    )
