#ifndef DINARA_APPROXIMATE_COLORING_HPP
#define DINARA_APPROXIMATE_COLORING_HPP
...
template<class Graph> void dinara::approximateColoring(
    const Graph& graph,
...
)
{
    ...
    for(uint64_t iteration=0; iteration<n; iteration++) {
        ...
        DINARA_ASSERT(not verticesBySaturationDegree.empty());
    ...
    DINARA_ASSERT(find(color.begin(), color.end(), noColor) == color.end());
