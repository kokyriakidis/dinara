#ifndef DINARA_SHORTEST_PATH_BOUNDED_DISTANCE_HPP
#define DINARA_SHORTEST_PATH_BOUNDED_DISTANCE_HPP
...
template<class Graph, class DistanceType> DistanceType
    dinara::shortestPathBoundedDistance(
        const Graph& graph,
        typename Graph::vertex_descriptor source,
        typename Graph::vertex_descriptor destination,
        DistanceType maxPathLength,
        ...
        if(u == Graph::null_vertex()) {
         ...
         if(u == destination) {
             ...
             while(u != source) {
                 ...
                 DINARA_ASSERT(ite != edgeLength.end());
...
inline void dinara::testShortestPathBoundedDistance()
{
