        ...
        if(isSelfComplementary) {
            ...
            for(const EdgeId edgeId: reverseComplementedChain) {
            	...
            	if(wasFound[edgeId]) {
            		...
            		DINARA_ASSERT(chain.size() == reverseComplementedChain.size());
...
void Assembler::createAssemblyGraphVertices()
{
    ...
    // Check that we have what we need.
    DINARA_ASSERT(assemblyGraph.edgeLists.isOpen());
    checkMarkerGraphEdgesIsOpen();
    ...
    for(EdgeId age=0; age<assemblyGraph.edgeLists.size(); age++) {
        ...
        DINARA_ASSERT(it1 != vertexMap.end());
...
{
    DINARA_ASSERT(assemblyGraphPointer);
    AssemblyGraph& assemblyGraph = *assemblyGraphPointer;
    ...
    for(AssemblyGraph::EdgeId edgeId=0; edgeId!=assemblyGraph.edges.size(); edgeId++) {
        ...
        DINARA_ASSERT(assemblyGraph.reverseComplementEdge[edgeIdRc] == edgeId);
        ...
        DINARA_ASSERT(markerGraphEdgesRc.size() == n);
        for(uint64_t i=0; i<n; i++) {
            ...
            DINARA_ASSERT(markerGraph.reverseComplementEdge[markerGraphEdgeId] == markerGraphEdgeIdRc);
            DINARA_ASSERT(markerGraph.reverseComplementEdge[markerGraphEdgeIdRc] == markerGraphEdgeId);
        }
    }
#endif
    ...
    uint32_t storeCoverageDataCsvLengthThreshold)
{
    ...
    if(storeCoverageDataCsvLengthThreshold > 0) {
         DINARA_ASSERT(markerGraph.vertexCoverageData.isOpen());
         DINARA_ASSERT(markerGraph.edgeCoverageData.isOpen());
    }
    ...
    if(assembleData.storeCoverageDataCsvLengthThreshold > 0) {
        DINARA_ASSERT(std::filesystem::create_directory("Coverage"));
    }
    ...
    for(AssemblyGraph::EdgeId edgeId=0; edgeId<assemblyGraph.edgeLists.size(); edgeId++) {
        ...
        DINARA_ASSERT(p != uninitializedPair);
...
void Assembler::computeAssemblyStatistics()
{
    ...
    // Check that we have what we need.
    DINARA_ASSERT(assemblyGraph.vertices.isOpen);
    const size_t vertexCount = assemblyGraph.vertices.size();
    DINARA_ASSERT(assemblyGraph.edges.isOpen);
    const size_t edgeCount = assemblyGraph.edges.size();
    DINARA_ASSERT(assemblyGraph.sequences.isOpen());
    DINARA_ASSERT(assemblyGraph.sequences.size() == edgeCount);
    DINARA_ASSERT(assemblyGraph.repeatCounts.isOpen());
    DINARA_ASSERT(assemblyGraph.repeatCounts.size() == edgeCount);
...
void Assembler::writeGfa1(const string& fileName)
{
    ...
    for(EdgeId edgeId=0; edgeId<assemblyGraph.sequences.size(); edgeId++) {
        ...
        DINARA_ASSERT(sequence.baseCount == repeatCounts.size());
        gfa << "S\t" << edgeId << "\t";
    ...
    for(VertexId vertexId=0; vertexId<assemblyGraph.vertices.size(); vertexId++) {
        ...
        for(const EdgeId edge0: edges0) {
            ...
            if(assemblyGraph.isAssembledEdge(edge0)) {
                ...
                DINARA_ASSERT(assemblyGraph.isAssembledEdge(edge0Rc));
            ...
            for(const EdgeId edge1: edges1) {
                ...
                if(assemblyGraph.isAssembledEdge(edge1)) {
                    ...
                    DINARA_ASSERT(assemblyGraph.isAssembledEdge(edge1Rc));
...
void Assembler::writeGfa1BothStrands(const string& fileName)
{
    ...
    for(EdgeId edgeId=0; edgeId<assemblyGraph.sequences.size(); edgeId++) {
        ...
        if(assemblyGraph.isAssembledEdge(edgeId)) {
            ...
            DINARA_ASSERT(assemblyGraph.isAssembledEdge(edgeIdRc));
    ...
    )
{
    ...
    DINARA_ASSERT(repeatCounts1.size() == k);
    ...
    LocalAssemblyGraph& graph) const
{
    ...
    BGL_FORALL_VERTICES(v0, graph, LocalAssemblyGraph) {
        ...
        for(uint64_t edgeId: childEdges) {
            ...
            DINARA_ASSERT(vertexId1 < assemblyGraph.vertices.size());
