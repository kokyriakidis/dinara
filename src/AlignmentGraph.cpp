    ...
    size_t maxDrift)
{
    ...
    for(vertex_iterator itA=itBegin; itA!=itEnd; ++itA) {
        ...
        DINARA_ASSERT(correctedOrdinalA0 < int(markerCount0));
        DINARA_ASSERT(correctedOrdinalA1 < int(markerCount1));
        ...
        for(; itB!=itEnd; ++itB) {
            ...
            DINARA_ASSERT(ordinalB0 >= ordinalA0);
            const int correctedOrdinalB0 = int(correctedOrdinals[0][ordinalB0]);
            DINARA_ASSERT(correctedOrdinalB0 < int(markerCount0));
            ...
            DINARA_ASSERT(correctedOrdinalB1 < int(markerCount1));
    ...
    const string& fileName)
{
    if(displayAlignmentMatrixByGlobalFrequency) {
        DINARA_ASSERT(kmerCounter);
        DINARA_ASSERT(kmerCounter->isAvailable());
    }
