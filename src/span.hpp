#ifndef DINARA_SPAN_HPP
#define DINARA_SPAN_HPP
...
inline uint64_t dinara::atoul(const std::span<const char>& s)
{
    uint64_t n = 0;
    for(uint64_t i=0; ; i++) {
        const char c = s[i];
...
inline std::string dinara::convertToString(const std::span<const char>& m)
{
    auto view = std::string_view(m.data(), m.size());
    return string(view);
}
inline std::string dinara::convertToString(const std::span<char>& m)
{
    auto view = std::string_view(m.data(), m.size());
    return string(view);
}
