    ...
    ostream& html)
{
    ...
    for(const CommonKmerInfo& commonKmerInfo: activeKmerInfos) {
        DINARA_ASSERT(commonKmerInfo.rank0 == commonKmerInfo.rank1);
    }
    ...
    alignment.clear();
    DINARA_ASSERT(activeKmerInfos.size() > 1);
    ...
    // This alignment is constrained on the right only.
    {
        ...
        if(ordinalB0 > 0 and ordinalB1 > 0) {
            ...
            DINARA_ASSERT(ordinal0 == ordinalB0);
            DINARA_ASSERT(ordinal1 == ordinalB1);
        }
    }
    ...
    for(uint64_t step=1; step<activeKmerInfos.size(); step++) {
        ...
        DINARA_ASSERT(commonKmerInfoB.rank0 > commonKmerInfoA.rank0);
        DINARA_ASSERT(commonKmerInfoB.rank1 > commonKmerInfoA.rank1);
        ...
        DINARA_ASSERT(not (alignmentScore == seqan::MinValue<int64_t>::VALUE));
