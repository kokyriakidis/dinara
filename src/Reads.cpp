...
pair<Base, uint8_t> Reads::getOrientedReadBaseAndRepeatCount(
    ...
    uint32_t position) const
{
    DINARA_ASSERT(representation == 1);
...
{
    DINARA_ASSERT(readId < readMetaData.size());
...
void Reads::writeReadLengthHistogram(const string& fileName) {
    ...
    n50 = 0;
    {
        ...
        DINARA_ASSERT(cumulativeReadCount == 0);
        DINARA_ASSERT(cumulativeBaseCount == 0);
    }
...
void Reads::findDuplicates(const string& handleDuplicates)
{
    ...
    DINARA_ASSERT(readFlags.size() == readCount);
    DINARA_ASSERT(readNames.size() == readCount);
