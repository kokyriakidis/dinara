...
BidirectedAnchorGraph Shasta2AssemblyGraph::toBidirected(
    const vector<AnchorWindow>& anchorWindows,
    const Shasta2Journeys& journeys) const
{
    const Shasta2AssemblyGraph& assemblyGraph = *this;

    // Find max anchor ID to size the bidirected graph.
    uint64_t maxAnchorId = 0;
    BGL_FORALL_VERTICES(v, assemblyGraph, Shasta2AssemblyGraph) {
        maxAnchorId = max(maxAnchorId, uint64_t(assemblyGraph[v].anchorId));
    }
    const uint64_t bidirNodeCount = (maxAnchorId / 2) + 1;

    BidirectedAnchorGraph bg;
    bg.resize(bidirNodeCount);

    // Set node properties from vertices.
    BGL_FORALL_VERTICES(v, assemblyGraph, Shasta2AssemblyGraph) {
        const auto& vertex = assemblyGraph[v];
        const auto bid = BidirectedAnchorId::fromDirected(vertex.anchorId);
        if(vertex.windowId != Shasta2AssemblyGraphVertex::noWindow) {
            bg.setNodeWindow(bid, vertex.windowId);
            if(vertex.windowId < anchorWindows.size()) {
                bg.setNodeBackboneRead(bid,
                    uint32_t(anchorWindows[vertex.windowId].backboneOrientedReadId.getReadId()));
            }
        }
    }
    ...
    BGL_FORALL_EDGES(e, assemblyGraph, Shasta2AssemblyGraph) {
        ...
        const auto dstId = assemblyGraph[target(e, assemblyGraph)].anchorId;
        ...
        props.coverage = uint64_t(edge.averageCoverage());
    ...
    // Normalize orientations: prefer backbone strand-0 as forward.
    ...
    for(const auto& window : anchorWindows) {
        const auto& positions = window.filteredBackbonePositions.empty()
            ? [&]() -> const vector<uint32_t>& {
                static vector<uint32_t> allPositions;
                allPositions.clear();
    ...
    cout << "Shasta2AssemblyGraph::toBidirected: " << activeNodes
         << " vertices, " << activeEdges << " edges -> "
         << bidirNodeCount << " bidirected nodes." << endl;
