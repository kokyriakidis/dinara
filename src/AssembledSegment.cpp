...
void AssembledSegment::computeVertexOffsets()
{
    ...
    for(size_t i=0; i<edgeCount; i++) {
        ...
        if(overlap > 0) {
            DINARA_ASSERT(edgeSequences[i].empty());
            DINARA_ASSERT(edgeRepeatCounts[i].empty());
...
void AssembledSegment::computeVertexAssembledPortion()
{
    ...
    for(int i=0; i<int(vertexCount); i++) {
        ...
        DINARA_ASSERT(vertexAssembledPortion[i].second <= k);
    }
}
    ...
    uint32_t end) const
{
    ...
    for(size_t i=0; ; i++) {
        ...
        if(boost::icl::intersects(
            ...
            DINARA_ASSERT(edgeRepeatCount.size() == edgeSequenceLength);
            // const uint32_t maxEdgeRepeatCount =
            //    *std::max_element(edgeRepeatCount.begin(), edgeRepeatCount.end());
            // DINARA_ASSERT(maxEdgeRepeatCount < 10);  // For now. Add additional code when this fails.
            html <<
                "<tr><td>Edge"
