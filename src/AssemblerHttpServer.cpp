...
namespace dinara {
    std::map<string, string> outputDescriptionTable = {
        ...
        {
            "Data",
            "Directory containing Dinara binary data."
        },
        {
...
#define DINARA_ADD_TO_FUNCTION_TABLE(name) httpServerData.functionTable[string("/") + #name ] = &Assembler::name
...
void Assembler::fillServerFunctionTable()
{
    ...
    DINARA_ADD_TO_FUNCTION_TABLE(exploreSummary);
    DINARA_ADD_TO_FUNCTION_TABLE(exploreRead);
    DINARA_ADD_TO_FUNCTION_TABLE(exploreLookupRead);
    DINARA_ADD_TO_FUNCTION_TABLE(exploreReadSequence);
    DINARA_ADD_TO_FUNCTION_TABLE(exploreReadMarkers);
    DINARA_ADD_TO_FUNCTION_TABLE(exploreMarkerKmers);
    DINARA_ADD_TO_FUNCTION_TABLE(blastRead);
    DINARA_ADD_TO_FUNCTION_TABLE(exploreAlignments);
    DINARA_ADD_TO_FUNCTION_TABLE(exploreAlignmentCoverage);
    DINARA_ADD_TO_FUNCTION_TABLE(exploreAlignment);
    DINARA_ADD_TO_FUNCTION_TABLE(computeAllAlignments);
    DINARA_ADD_TO_FUNCTION_TABLE(exploreAlignmentCandidateGraph);
    DINARA_ADD_TO_FUNCTION_TABLE(exploreAlignmentGraph);
...
void Assembler::writeAssemblySummaryBody(ostream& html)
{
    ...
    html <<
        "<h1>Dinara assembly summary</h1>"
        "<h3>Dinara version</h3>" <<
        buildId() <<
...
{
    writeHtmlBegin(html);
    html << "<body><h1>Dinara assembly output files</h1><table>";

    // Loop over files in the assembly directory,
    // in case-insensitive alphabetic order.
    vector<string> assemblyFiles = dinara::filesystem::directoryContents(".");
...
void dinara::writeStrandSelection(
