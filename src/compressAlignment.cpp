...
void dinara::compress(const Alignment& alignment, string& s)
{
    s.clear();
    uint32_t ordinal0 = 0;
    uint32_t ordinal1 = 0;
    ...
    for(uint64_t i=0; i<alignment.ordinals.size(); ) {
        ...
        if(Format0::ok(skip0, skip1, n)) {
            ...
            throw std::runtime_error("Dinara Internal Error: Unable to compress alignment streak: "
...
void dinara::decompress(span<const char> s, Alignment& alignment)
{
    alignment.clear();
    uint32_t ordinal0 = 0;
    uint32_t ordinal1 = 0;
...
uint8_t dinara::compressAlignment::extractFormatIdentifier(const char c) {
    if ((c & Format0::idMask) == Format0::id) {
        ...
        string errorStr("Dinara Internal Error: Unsupported format in byte - ");
...
void dinara::testAlignmentCompression() {
    ...
    string compressed;
    dinara::compress(alignment, compressed);
    ...
    dinara::decompress(spanOfBytes, decompressedAlignment);
    ...
    DINARA_ASSERT(alignment.ordinals == decompressedAlignment.ordinals);
}
