...
PYBIND11_MODULE(dinara, dinaraModule)
{

    // Expose class OrientedReadPair to Python.
    class_<OrientedReadPair>(dinaraModule, "OrientedReadPair")
    ...
    class_<Reads>(dinaraModule, "Reads")
    ...
    class_<AlignOptions>(dinaraModule, "AlignOptions")
        .def(pybind11::init<>())
        ...
        .def_readwrite("maxErrorRate", &AlignOptions::maxErrorRate)
        .def_readwrite("overlapDpMatchScore", &AlignOptions::overlapDpMatchScore)
        .def_readwrite("overlapDpMismatchScore", &AlignOptions::overlapDpMismatchScore)
        .def_readwrite("overlapDpGapOpen1", &AlignOptions::overlapDpGapOpen1)
        .def_readwrite("overlapDpGapExtend1", &AlignOptions::overlapDpGapExtend1)
        .def_readwrite("overlapDpGapOpen2", &AlignOptions::overlapDpGapOpen2)
        .def_readwrite("overlapDpGapExtend2", &AlignOptions::overlapDpGapExtend2)
    ...
    class_<Mode2AssemblyOptions>(dinaraModule, "Mode2AssemblyOptions")
        .def(pybind11::init<>())
    ...
    class_<Assembler>(dinaraModule, "Assembler")
    ...
    class_<AssembledSegment>(dinaraModule, "AssembledSegment")
    ...
    class_<CompressedCoverageData>(dinaraModule, "CompressedCoverageData")
    ...
    class_<AssemblerOptions>(dinaraModule, "AssemblerOptions")
        .def(pybind11::init<const string&>())
        .def_readonly("assemblyOptions", &AssemblerOptions::assemblyOptions)
        ;
    class_<AssemblyOptions>(dinaraModule, "AssemblyOptions")
        .def_readonly("mode3Options", &AssemblyOptions::mode3Options)
        ;
    class_<Mode3AssemblyOptions>(dinaraModule, "Mode3AssemblyOptions");



    // Expose class mode3AssemblyGraph to Python.
    class_<mode3::AssemblyGraph>(dinaraModule, "Mode3AssemblyGraph")
        .def(pybind11::init<const string&, uint64_t, const Assembler&, const Mode3AssemblyOptions&>());


    // Constants.
    dinaraModule.attr("invalidGlobalMarkerGraphVertexId") = MarkerGraph::invalidVertexId;
    dinaraModule.attr("invalidCompressedGlobalMarkerGraphVertexId") =
        uint64_t(MarkerGraph::invalidCompressedVertexId);
    dinaraModule.attr("configurationTable") = dinara::configurationTable;



    // Non-member functions exposed to Python.
    dinaraModule.def("openPerformanceLog",
        openPerformanceLog
        );
    dinaraModule.def("testMultithreadedObject",
        testMultithreadedObject
        );
    dinaraModule.def("testMemoryMappedVector",
        testMemoryMappedVector
        );
    dinaraModule.def("testBase",
        testBase
        );
    dinaraModule.def("testShortBaseSequence",
        testShortBaseSequence
        );
    dinaraModule.def("testLongBaseSequence",
        testLongBaseSequence
        );
    dinaraModule.def("testSplitRange",
        testSplitRange
        );
    dinaraModule.def("testCompactUndirectedGraph1",
        testCompactUndirectedGraph1
        );
    dinaraModule.def("testCompactUndirectedGraph2",
        testCompactUndirectedGraph1
        );
    dinaraModule.def("testSpoa",
        testSpoa
        );
    dinaraModule.def("testSimpleBayesianConsensusCaller",
        testSimpleBayesianConsensusCaller
        );
    dinaraModule.def("testMedianConsensusCaller",
        testMedianConsensusCaller
        );
    dinaraModule.def("testDeduplicateAndCount",
        testDeduplicateAndCount
        );
    dinaraModule.def("testBitReversal",
        testBitReversal
        );
    dinaraModule.def("dset64Test",
        dset64Test,
        arg("n"),
        arg("m"),
        arg("threadCount"),
    ...
    dinaraModule.def("mappedCopy",
        mappedCopy
        );
    dinaraModule.def("testLongBaseSequence",
        testLongBaseSequence
        );
    dinaraModule.def("testAlignmentCompression",
        testAlignmentCompression
        );
    dinaraModule.def("testMemoryMappedAllocator",
        MemoryMapped::testMemoryMappedAllocator
        );
    dinaraModule.def("testLapack",
        testLapack
        );
    dinaraModule.def("testDiploidBayesianPhase",
        testDiploidBayesianPhase
        );
    dinaraModule.def("testSubsetGraph",
        testSubsetGraph
        );
    dinaraModule.def("testLongestPath",
        testLongestPath
        );
    dinaraModule.def("testEnumeratePaths",
        testEnumeratePaths
        );
    dinaraModule.def("globalMsaPython",
        globalMsaPython
        );
}
