    ...
    size_t threadCount)
{
    ...
    DINARA_ASSERT(markerGraph.edges.isOpenWithWriteAccess);
    ...
    for(VertexId v=0; v!=vertexCount; v++) {
        if(markerGraph.outDegree(v) == 0) {
            ...
            DINARA_ASSERT(markerGraph.inDegree(vRc) == 0);
...
void Assembler::splitMarkerGraphSecondaryEdgesThreadFunction(uint64_t threadId)
{
    ...
    while(getNextBatch(begin, end)) {
        ...
        for(MarkerGraphEdgeId edgeId=begin; edgeId!=end; edgeId++) {
            ...
            DINARA_ASSERT(edge.isSecondary == edgeRc.isSecondary);
            ...
            DINARA_ASSERT(markerIntervals.size() == markerIntervalsRc.size());
            for(uint64_t i=0; i<markerIntervals.size(); i++) {
                ...
                DINARA_ASSERT(orientedReadId.getReadId() == orientedReadIdRc.getReadId());
                DINARA_ASSERT(orientedReadId.getStrand() == 1 - orientedReadIdRc.getStrand());
            }
