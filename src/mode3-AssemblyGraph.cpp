    ...
    anchorIds(anchorIds)
{
    ...
    DINARA_ASSERT(componentId == componentIdArgument);
}
    ...
    anchorIds(anchorIds)
{
    ...
    for(const vector<AnchorId>& anchorChain: anchorChains) {
        DINARA_ASSERT(anchorChain.size() >= 2);
    ...
    bool debug)
{
    ...
    DINARA_ASSERT(std::is_sorted(orientedReadIds.begin(), orientedReadIds.end()));
    DINARA_ASSERT(std::is_sorted(anchorIds.begin(), anchorIds.end()));
...
void AssemblyGraph::removeVertex(vertex_descriptor cv)
{
    ...
    DINARA_ASSERT(in_degree(cv, assemblyGraph) == 0);
    DINARA_ASSERT(out_degree(cv, assemblyGraph) == 0);
    ...
    BGL_FORALL_EDGES(ce, assemblyGraph, AssemblyGraph) {
        ...
        for(uint64_t positionInBubbleChain=0; positionInBubbleChain<bubbleChain.size(); positionInBubbleChain++) {
            ...
            for(const Chain& chain: bubble) {
                DINARA_ASSERT(chain.front() == firstChain.front());
                DINARA_ASSERT(chain.back() == firstChain.back());
            }
...
{
    if(includeSequence) {
        DINARA_ASSERT(useSequenceLength);
    }
    ...
    uint64_t maxOffset2)    // Compared against the offset between entry and exit
{
    ...
    for(uint64_t superbubbleId=0; superbubbleId<superbubbles.size(); superbubbleId++) {
        ...
        DINARA_ASSERT(superbubble.size() > 1);
    ...
    std::set<vertex_descriptor>& previousSuperbubblesVertices)
{
        ...
        if(canDo) {
            ...
            if(debug) {
                ...
                for(uint64_t i=0; i<2; i++) {
                    ...
                    copy(chain.sequence.begin(), chain.sequence.end(), ostream_iterator<dinara::Base>(cout));
                    cout << "\n";
                }
            }
    ...
    DINARA_ASSERT(entranceBubble.size() == 0 or entranceBubble.size() == 2);
    DINARA_ASSERT(exitBubble.size() == 0 or exitBubble.size() == 2);
    ...
    ) const
{
    ...
    for(uint64_t i0=0; i0<inEdges.size(); i0++) {
        ...
        DINARA_ASSERT(bubble0.isHaploid());
        const Chain& chain0 = bubble0.front();
        DINARA_ASSERT(chain0.size() >= 2);
        ...
        for(uint64_t i1=0; i1<outEdges.size(); i1++) {
            ...
            DINARA_ASSERT(bubble1.isHaploid());
            const Chain& chain1 = bubble1.front();
            DINARA_ASSERT(chain1.size() >= 2);
    ...
    vertex_descriptor cv, bool debug)
{
    ...
    for(uint64_t i0=0; i0<inEdges.size(); i0++) {
        ...
        for(uint64_t i1=0; i1<outEdges.size(); i1++) {
            ...
            DINARA_ASSERT(newBubbleLast.size() == 1);
            Chain& newChainLast = newBubbleLast.front();
            DINARA_ASSERT(newChainLast.back() == cGraph[cv].getAnchorId());
            newChainLast.resize(newChainLast.size() - 1);

            // Append chain1, except for cv.
            DINARA_ASSERT(chain1.front() == cGraph[cv].getAnchorId());
    ...
    double minLogP)
{
    ...
    BGL_FORALL_INEDGES(cv, ce, cGraph, AssemblyGraph) {
        const BubbleChain& bubbleChain = cGraph[ce];
        DINARA_ASSERT(bubbleChain.lastBubble().isHaploid());
        inEdges.push_back(ce);
    }
    DINARA_ASSERT(inEdges.size() == 2);

    // Gather the out-edges.
    vector<edge_descriptor> outEdges;
    BGL_FORALL_OUTEDGES(cv, ce, cGraph, AssemblyGraph) {
        const BubbleChain& bubbleChain = cGraph[ce];
        DINARA_ASSERT(bubbleChain.firstBubble().isHaploid());
        outEdges.push_back(ce);
    }
    DINARA_ASSERT(outEdges.size() == 2);
    ...
    DINARA_ASSERT(inEdges[1] == outEdges[1]);
    ...
    DINARA_ASSERT(cGraph[inEdge].isSimpleChain());
    DINARA_ASSERT(cGraph[outEdge].isSimpleChain());
    DINARA_ASSERT(cGraph[cycleEdge].isSimpleChain());
    ...
    if(isInPhase) {
        ...
        DINARA_ASSERT(newBubbleChain.isSimpleChain());
    ...
    double minLogP)
{
    ...
    DINARA_ASSERT(detangleToleranceHigh > detangleToleranceLow);
    ...
    uint64_t n)
{
    ...
    BubbleChain& bubbleChain = cGraph[ce];
    DINARA_ASSERT(bubbleChain.isSimpleChain());
    ...
    uint64_t n)
{
    ...
    DINARA_ASSERT(bubbleChain0.isSimpleChain());
    DINARA_ASSERT(bubbleChain1.isSimpleChain());
    ...
    DINARA_ASSERT(first0 < chain0.size());
    DINARA_ASSERT(last0 < chain0.size());
    ...
    DINARA_ASSERT(first1 < chain1.size());
    DINARA_ASSERT(last1 < chain1.size());
    ...
    DINARA_ASSERT(bestCommonCount > 0);
    DINARA_ASSERT(i0Best != invalid<uint64_t>);
    DINARA_ASSERT(i1Best != invalid<uint64_t>);
    ...
    uint64_t longBubbleThreshold)
{
    ...
    for(uint64_t i=0; /* Check later */; i++) {
        ...
        for(const auto& p: phasedComponent) {
            ...
            DINARA_ASSERT(phase==1 or phase==-1);
            const Bubble& bubble = bubbleChain[positionInBubbleChain];
            DINARA_ASSERT(bubble.isDiploid());
    ...
    uint64_t longBubbleThreshold)
{
    ...
    for(const auto& phasedComponentPointer: phasingTable.phasedComponents) {
        ...
        for(uint64_t i=1; i<phasedComponent.size(); i++) {
            ...
            DINARA_ASSERT(bubble0.isDiploid());
            DINARA_ASSERT(bubble1.isDiploid());
    ...
    vector<OrientedReadId>& orientedReadIds) const
{
    ...
    DINARA_ASSERT(first < chain.size());
    DINARA_ASSERT(last < chain.size());
...
void AssemblyGraph::PhasingGraph::phase(bool debug)
{
    ...
    for(const  pair<uint64_t, uint64_t>& p: componentTable) {
        ...
        const vector<vertex_descriptor>& component = components[componentId];
        DINARA_ASSERT(component.size() == p.second);
        if(debug) {
...
void AssemblyGraph::PhasingGraph::phase1(bool debug, bool useBayesianModel)
{
    ...
    while(true) {
        ...
        DINARA_ASSERT(component.size() == largestComponentSize);
        if(debug) {
...
bool AssemblyGraph::PhasingGraph::isConsistent(edge_descriptor e) const
{
    ...
    DINARA_ASSERT(phase0==+1 or phase0==-1);
    DINARA_ASSERT(phase1==+1 or phase1==-1);
    DINARA_ASSERT(phase==+1 or phase==-1);
...
void AssemblyGraph::writeAssemblyDetails() const
{
    ...
    BGL_FORALL_EDGES(e, assemblyGraph, AssemblyGraph) {
        ...
        for(uint64_t positionInBubbleChain=0; positionInBubbleChain<bubbleChain.size(); positionInBubbleChain++) {
            ...
            for(uint64_t indexInBubble=0; indexInBubble<bubble.size(); indexInBubble++) {
                const Chain& chain = bubble[indexInBubble];
                DINARA_ASSERT(chain.wasAssembled);
                DINARA_ASSERT(chain.stepSequences.size() == chain.size() - 1);
                ...
                for(uint64_t positionInChain=0; /* Check later */ ; positionInChain++) {
                    ...
                    // If this was the last primary edge for the chain, we are done.
                    if(positionInChain == chain.size() - 1) {
                        DINARA_ASSERT(positionInSequence == chain.sequence.size());
                        break;
                    }
    ...
    AssemblyGraph::cloneAndTruncateAtEnd(bool debug, edge_descriptor ce)
{
    ...
    // Sanity checks.
    DINARA_ASSERT(not bubbleChain.empty());
    DINARA_ASSERT(bubbleChain.lastBubble().isHaploid());
    ...
    if(bubbleChain.size() == 1) {
        ...
        DINARA_ASSERT(bubble.isHaploid());
        const Chain& chain = bubble.front();
        ...
        newEdge.id = nextEdgeId++;
        BubbleChain& newBubbleChain = newEdge;
        DINARA_ASSERT(newBubbleChain.size() == 1);
        Bubble& newBubble = newBubbleChain.lastBubble();
        DINARA_ASSERT(newBubble.isHaploid());
        Chain& newChain = newBubble.front();
        DINARA_ASSERT(chain.size() > 2);
    ...
    // Case where the bubble chain consists of more than one bubble.
        ...
        DINARA_ASSERT(lastBubble.isHaploid());
        const Chain& lastChain = lastBubble.front();
        DINARA_ASSERT(lastChain.size() > 1);
        ...
        DINARA_ASSERT(newLastBubble.isHaploid());
    ...
    AssemblyGraph::cloneAndTruncateAtBeginning(bool debug, edge_descriptor ce)
{
    ...
    // Case where the bubble chain consists of more than one bubble.
        ...
        DINARA_ASSERT(firstBubble.isHaploid());
        const Chain& firstChain = firstBubble.front();
        DINARA_ASSERT(firstChain.size() > 1);
        ...
        DINARA_ASSERT(newFirstBubble.isHaploid());
    ...
    uint64_t k)
{
    ...
    if(totalAddedEdgesCount == 0) {
        DINARA_ASSERT(totalRemovedEdgesCount == 0);
        if(debug) {
    ...
    dinara::lengauer_tarjan_dominator_tree(chainGraph, 0,
    ...
    while(true) {
        ...
        DINARA_ASSERT(v != invalid<uint64_t>);
    }
    if(debug) {
    ...
    DINARA_ASSERT(in_degree(0, chainGraph) == 0);
    DINARA_ASSERT(out_degree(0, chainGraph) == 1);
    DINARA_ASSERT(in_degree(chain.size()-1, chainGraph) == 1);
    DINARA_ASSERT(out_degree(chain.size()-1, chainGraph) == 0);
    for(uint64_t i=1; i<chain.size()-1; i++) {
    ...
    while(true) {
        ...
        // Move forward.
        DINARA_ASSERT(out_degree(v, chainGraph) == 1);
    ...
    for(uint64_t i=1; i<newPath.size(); i++) {
        DINARA_ASSERT(newPath[i] > newPath[i-1]);
    }
...
void AssemblyGraph::splitTerminalHaploidBubbles(edge_descriptor ce)
{
    ...
    if(splitFirstBubble and splitLastBubble) {
        DINARA_ASSERT(bubbleChain.size() > 2);
    }
                    ...
                    if(debug) {
                        for(uint64_t indexInBubble=0; indexInBubble<2; indexInBubble++) {
                            ...
                            cout << ">" << chainStringId(ce, positionInBubbleChain, indexInBubble) <<
                                " " << sequence.size() << "\n";
                            copy(sequence.begin(), sequence.end(), ostream_iterator<dinara::Base>(cout));
                            cout << "\n";
                        }
                    }
...
uint64_t AssemblyGraph::getOrientedReadIndex(OrientedReadId orientedReadId) const
{
    ...
    DINARA_ASSERT(*it == orientedReadId);
...
void AssemblyGraph::load(const string& assemblyStage, uint64_t componentIdArgument)
{
    ...
    // Sanity check.
    DINARA_ASSERT(componentIdArgument == componentId);
}
...
void AssemblyGraph::annotateAnchors()
{
    ...
    BGL_FORALL_VERTICES(v, assemblyGraph, AssemblyGraph) {
        ...
        DINARA_ASSERT(anchorIds[localAnchorId] == anchorId);
...
void AssemblyGraph::storeSuperbubblesInformation(const Superbubbles& superbubbles)
{
    ...
    for(uint64_t superbubbleId=0; superbubbleId<superbubbles.size(); superbubbleId++) {
        ...
        for(const vertex_descriptor v: superbubble) {
            ...
            DINARA_ASSERT(vertex.superbubbleId == invalid<uint64_t>);
