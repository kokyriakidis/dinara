...
uint64_t ReadGraph::getReverseComplementEdgeId(uint64_t edgeId) const
{
    ...
    DINARA_ASSERT(orientedReadIds == reverseComplementEdge.orientedReadIds);
    ...
    vector<OrientedReadId>& neighbors) const
{
    ...
    while(not q.empty()) {
        ...
        DINARA_ASSERT(it0 != distanceMap.end());
        const uint64_t distance0 = it0->second;
        const uint64_t distance1 = distance0 + 1;
        DINARA_ASSERT(distance1 <= maxDistance);
...
void ReadGraph::findBridges(vector<bool>& keepAlignment, uint64_t maxDistance)
{
    ...
    for(ReadId readId0=0; readId0<readCount; readId0++) {
        ...
        */
        ...
        for(uint64_t i1=0; i1<n; i1++) {
            ...
            for(const uint32_t edgeId12: connectivity[orientedReadId1.getValue()]) {
                ...
                DINARA_ASSERT(orientedReadId1 != orientedReadId2);
                ...
                DINARA_ASSERT(*it2 == orientedReadId2);
        ...
        */
        ...
        DINARA_ASSERT(largestComponent.size() == componentTable.front().second);
