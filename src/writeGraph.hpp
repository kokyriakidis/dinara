#ifndef DINARA_WRITE_GRAPH_HPP
#define DINARA_WRITE_GRAPH_HPP
...
template<class Graph> void dinara::WriteGraph::writeSvg(
    const Graph& graph,
    const string& svgId,
    uint64_t width,
    uint64_t height,
...
template<class Graph> void dinara::WriteGraph::writeOrderedSvg(
        const Graph& graph,
        const string& svgId,
        uint64_t width,
        uint64_t height,
