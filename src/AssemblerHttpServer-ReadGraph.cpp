...
void Assembler::exploreBidirectionalReadGraph(
    const vector<string>& request,
    ostream& html)
{
    // Check that the bidirectional read graph is available.
    try {
        checkBidirectionalReadGraphIsOpen();
    } catch(...) {
        html << "The bidirectional read graph is not available." << endl;
        return;
    }
    ...
    // Write the form.
    html <<
        "<h3>Display a local subgraph of the bidirectional read graph</h3>"
        "<form>"
    ...
    html <<
        ...
        // No "Allow cross-strand edges" checkbox — the BRG handles
        // cross-strand overlaps natively via orientation-aware traversal.
    ...
    // Create the local read graph via BRG BFS.
    LocalReadGraph graph;
    const bool created = createLocalBidirectionalReadGraph(
        readIds,
        maxDistance,
        allowChimericReads,
    ...
    html << "<p>The local bidirectional read graph has " << num_vertices(graph);
    ...
    if(addBlastAnnotations) {
        ...
        BGL_FORALL_VERTICES(v, graph, LocalReadGraph) {
            ...
            for(const auto& alignment : vertexAlignments) {
                DINARA_ASSERT(alignment.size() == 4);
    ...
    // Cross-strand BRG edges (isSameStrand==0) are drawn in orange.
    BGL_FORALL_EDGES(e, graph, LocalReadGraph) {
        if(graph[e].crossesStrands) {
            graph[e].color = "Orange";
        }
    }


    // Triangle analysis, if requested.
    // Uses BRG edge indices stored in LocalReadGraphEdge::globalEdgeId.
    ...
    // Least square analysis, if requested.
    ...
    // Write a title.
    html <<
        "<h1 style='line-height:10px'>Bidirectional read graph near oriented read(s) " <<
        readIdsString << "</h1>";
    ...
    if(alignmentAnalysis == "triangles" and not triangles.empty()) {
        ...
        for(const auto& p : triangles) {
            ...
            DINARA_ASSERT(target(e20, graph) == v0);
            ...
            // Look up alignment IDs from BRG edges.
            const BidirectionalReadGraphEdge& brgEdge01 =
                bidirectionalReadGraph.edges[graph[e01].globalEdgeId];
            const BidirectionalReadGraphEdge& brgEdge12 =
                bidirectionalReadGraph.edges[graph[e12].globalEdgeId];
            const BidirectionalReadGraphEdge& brgEdge20 =
                bidirectionalReadGraph.edges[graph[e20].globalEdgeId];

            const uint64_t alignmentId01 = brgEdge01.alignmentId;
            const uint64_t alignmentId12 = brgEdge12.alignmentId;
            const uint64_t alignmentId20 = brgEdge20.alignmentId;
