    ...
    LocalReadGraph& graph)
{
    ...
    while (!q.empty()) {
        ...
        for (const uint64_t i: readGraph.connectivity[orientedReadId0.getValue()]) {
            DINARA_ASSERT(i < readGraph.edges.size());
...
void Assembler::flagChimericReadsThreadFunction(size_t threadId)
{
    ...
    while(getNextBatch(begin, end)) {
        ...
        for(ReadId startReadId=ReadId(begin); startReadId!=ReadId(end); startReadId++) {
            ...
            DINARA_ASSERT(localVertices.empty());
...
void Assembler::computeReadGraphConnectedComponents() const
{
    ...
    DINARA_ASSERT(readGraph.connectivity.size() == orientedReadCount);
    ...
    bool allowInconsistentAlignmentEdges)
{
    ...
    DINARA_ASSERT(createLocalReadGraph(
...
void Assembler::flagCrossStrandReadGraphEdges1(int maxDistance, size_t threadCount)
{
   ...
   for(ReadId componentId=0; componentId!=orientedReadCount; componentId++) {
        ...
        for(size_t i=0; i<vertexCount; i+=2) {
            ...
            DINARA_ASSERT(orientedReadId0.getReadId() == orientedReadId1.getReadId());
            DINARA_ASSERT(orientedReadId0.getStrand() == 0);
            DINARA_ASSERT(orientedReadId1.getStrand() == 1);
        }
        ...
        for(size_t i=0; i<edgeIds.size(); i+=2){
            ...
            DINARA_ASSERT(alignmentId == edgeIds[i+1].second);
        ...
        for(const auto& p: edgePairs) {
            ...
            for(const uint32_t edgeId: edgeIds) {
                ...
                DINARA_ASSERT(component0 != component0rc);
                DINARA_ASSERT(component1 != component1rc);
...
void Assembler::flagCrossStrandReadGraphEdges2()
{
    ...
    DINARA_ASSERT((readGraph.edges.size() % 2) == 0);
    ...
    for(uint64_t edgeId=0; edgeId<readGraph.edges.size(); edgeId+=2) {
        const ReadGraphEdge& edge = readGraph.edges[edgeId];
        DINARA_ASSERT(not edge.crossesStrands);
        ...
        // this edge.
        {
            ...
            DINARA_ASSERT(not nextEdge.crossesStrands);
            ...
            DINARA_ASSERT(nextEdgeOrientedReadIds == edge.orientedReadIds);
            DINARA_ASSERT(nextEdge.alignmentId == alignmentId);
        }
    ...
    for(auto it=edgeTable.rbegin(); it!=edgeTable.rend(); ++it) {
        ...
        for(const uint64_t edgeId: v) {
            ...
            DINARA_ASSERT(A0.getReadId() == A1.getReadId());
            DINARA_ASSERT(B0.getReadId() == B1.getReadId());
            DINARA_ASSERT(A0.getStrand() == 1 - A1.getStrand());
            DINARA_ASSERT(B0.getStrand() == 1 - B1.getStrand());
    ...
    for(ReadId componentId=0; componentId<components.size(); componentId++) {
        ...
        const bool isSelfComplementary =
            component.size() > 1 &&
            (component[0].getReadId() == component[1].getReadId());
        DINARA_ASSERT(not isSelfComplementary);
...
{
    // Check that we have what we need.
    DINARA_ASSERT(alignmentData.isOpen);
    ...
    vector<double>& S) const
{
    ...
    DINARA_ASSERT(int(equations.size()) == M);
    ...
    */
    ...
    DINARA_ASSERT(fabs(S[N-1]) < 1.e-14);
    ...
    size_t threadCount)
{
    ...
    DINARA_ASSERT(readGraph.edges.isOpenWithWriteAccess);
    DINARA_ASSERT(readGraph.connectivity.isOpen());
