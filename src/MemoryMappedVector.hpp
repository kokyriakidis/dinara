...
#ifndef DINARA_MEMORY_MAPPED_VECTOR_HPP
#define DINARA_MEMORY_MAPPED_VECTOR_HPP
...
template<class T> class dinara::MemoryMapped::Vector {
public:
    ...
    {
        return dinara::touchMemory(begin(), end());
    }
    ...
    {
        DINARA_ASSERT(isOpen);
        DINARA_ASSERT(header);
...
private:
    ...
    class Header {
        ...
        {
            DINARA_ASSERT(requestedCapacity >= n);
            clear();
...
template<class T> inline size_t dinara::MemoryMapped::Vector<T>::size() const
{
    return isOpen ? header->objectCount : 0ULL;
}
template<class T> inline bool dinara::MemoryMapped::Vector<T>::empty() const
{
    return isOpen ? (size()==0) : 0ULL;
}
template<class T> inline size_t dinara::MemoryMapped::Vector<T>::capacity() const
{
    return isOpen ? header->capacity : 0ULL;
}

template<class T> inline T& dinara::MemoryMapped::Vector<T>::operator[](size_t i)
{
    // DINARA_ASSERT(isOpen);
    return data[i];
}
template<class T> inline const T& dinara::MemoryMapped::Vector<T>::operator[](size_t i) const
{
    // DINARA_ASSERT(isOpen);
    return data[i];
}

template<class T> inline T& dinara::MemoryMapped::Vector<T>::front()
{
    DINARA_ASSERT(isOpen);
    return *data;
}
template<class T> inline const T& dinara::MemoryMapped::Vector<T>::front() const
{
    DINARA_ASSERT(isOpen);
    DINARA_ASSERT(size() > 0);
    return *data;
}

template<class T> inline T& dinara::MemoryMapped::Vector<T>::back()
{
    DINARA_ASSERT(isOpen);
    return data[size() - 1ULL];
}
template<class T> inline const T& dinara::MemoryMapped::Vector<T>::back() const
{
    DINARA_ASSERT(isOpen);
    return data[size() - 1ULL];
}
template<class T> inline T* dinara::MemoryMapped::Vector<T>::begin()
{
    DINARA_ASSERT(isOpen);
    return data;
}
template<class T> inline const T* dinara::MemoryMapped::Vector<T>::begin() const
{
    DINARA_ASSERT(isOpen);
    return data;
}

template<class T> inline T* dinara::MemoryMapped::Vector<T>::end()
{
    DINARA_ASSERT(isOpen);
    return data + size();
}

template<class T> inline const T* dinara::MemoryMapped::Vector<T>::end() const
{
    DINARA_ASSERT(isOpen);
    return data + size();
}
template<class T> inline void dinara::MemoryMapped::Vector<T>::push_back(const T& t)
{
    DINARA_ASSERT(isOpen);
    resize(size()+1ULL);
    back() = t;
...
// Destructor.
template<class T> inline dinara::MemoryMapped::Vector<T>::~Vector()
{
    if(isOpen) {
...
// and with write access.
// Return the file descriptor.
template<class T> inline int dinara::MemoryMapped::Vector<T>::openNew(const string& name)
{
...
// Open the given existing file.
// Return the file descriptor.
template<class T> inline int dinara::MemoryMapped::Vector<T>::openExisting(const string& name, bool readWriteAccess)
{
    const int fileDescriptor = ::open(
        name.c_str(),
...
// Truncate the given file descriptor to the specified size.
template<class T> inline void dinara::MemoryMapped::Vector<T>::truncate(int fileDescriptor, size_t fileSize)
{
...
// Map to memory the given file descriptor for the specified size.
template<class T> inline void* dinara::MemoryMapped::Vector<T>::map(int fileDescriptor, size_t fileSize, bool writeAccess)
{
...
// Find the size of the file corresponding to an open file descriptor.
template<class T> inline size_t dinara::MemoryMapped::Vector<T>::getFileSize(int fileDescriptor)
{
...
// Actual capacity will be a bit larger due to rounding up to the next page boundary.
// The vector is stored in a memory mapped file with the specified name.
template<class T> inline void dinara::MemoryMapped::Vector<T>::createNew(
    const string& name,
    size_t pageSize,
    size_t n,
    size_t requiredCapacity)
{
    DINARA_ASSERT(pageSize==4096 || pageSize==2*1024*1024);
...
template<class T> inline void dinara::MemoryMapped::Vector<T>::createNewAnonymous(
    size_t pageSize,
    size_t n,
    size_t requiredCapacity)
{
...
// Open a previously created vector with read-only or read-write access.
template<class T> inline void dinara::MemoryMapped::Vector<T>::accessExisting(const string& name, bool readWriteAccess)
{
    try {
        // If already open, should have called close first.
        DINARA_ASSERT(!isOpen);
        ...
        if(header->objectSize != sizeof(T)) {
            throw runtime_error("Error accessing " + name +
                ...
                ". You may be attempting to access an assembly created by a different version of Dinara."
                );
        }
...
template<class T> inline void dinara::MemoryMapped::Vector<T>::accessExistingReadOnly(const string& name)
{
    accessExisting(name, false);
}
template<class T> inline
    void dinara::MemoryMapped::Vector<T>::accessExistingReadWrite(
        const string& name)
{
...
// If this fails, create the vector, empty, and access it with read write access.
template<class T> inline void dinara::MemoryMapped::Vector<T>::accessExistingReadWriteOrCreateNew(
    const string& name,
    size_t pageSize)
{
    try {
        accessExistingReadWrite(name);
        DINARA_ASSERT(pageSize == header->pageSize);
    } catch(...) {
...
// Sync the mapped memory to disk.
template<class T> inline void dinara::MemoryMapped::Vector<T>::syncToDisk()
{
    DINARA_ASSERT(isOpen);
...
// Unmap the memory.
template<class T> inline void dinara::MemoryMapped::Vector<T>::unmap()
{
    DINARA_ASSERT(isOpen);
...
template<class T> inline void dinara::MemoryMapped::Vector<T>::unmapAnonymous()
{
    DINARA_ASSERT(isOpen);
...
// Sync the mapped memory to disk, then unmap it.
template<class T> inline void dinara::MemoryMapped::Vector<T>::close()
{
    DINARA_ASSERT(isOpen);
    syncToDisk();
    unmap();
...
template<class T> inline void dinara::MemoryMapped::Vector<T>::remove()
{
    if(fileName.empty()) {
        unmapAnonymous();
    } else {
...
// Resize works as for std::vector.
template<class T> inline void dinara::MemoryMapped::Vector<T>::resize(size_t newSize)
{
    DINARA_ASSERT(isOpenWithWriteAccess);
...
template<class T> inline void
    dinara::MemoryMapped::Vector<T>::resizeAnonymous(size_t newSize)
{
    const size_t oldSize = size();
    if(newSize == oldSize) {
...
template<class T> inline void dinara::MemoryMapped::Vector<T>::reserve()
{
    DINARA_ASSERT(isOpenWithWriteAccess);
    reserve(size());
}
...
template<class T> inline void dinara::MemoryMapped::Vector<T>::reserve(size_t capacity)
{
    DINARA_ASSERT(isOpenWithWriteAccess);
    DINARA_ASSERT(capacity >= size());
...
template<class T> inline
    void dinara::MemoryMapped::Vector<T>::reserveAnonymous(
    size_t capacity)
{
...
template<class T> inline void dinara::MemoryMapped::Vector<T>::unreserve()
{
    reserve(size());
}
...
// Make a copy of the Vector.
template<class T> inline void dinara::MemoryMapped::Vector<T>::makeCopy(
...
template<class T> inline void dinara::MemoryMapped::Vector<T>::rename(const string& newFileName)
{
    DINARA_ASSERT(isOpen);

    if(fileName.empty()) {
        DINARA_ASSERT(newFileName.empty());
    } else {
