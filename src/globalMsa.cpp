    ...
    )
{
    ...
    // Sanity check.
    DINARA_ASSERT(not sequences.empty());

    using Kmer = ShortBaseSequence64;
    DINARA_ASSERT(kmerLength <= Kmer::capacity);
    ...
    for(uint64_t i=0; i<sequences.size(); i++) {
        ...
        DINARA_ASSERT(kmerInfos.size() == sequence.size() - kmerLength + 1);
    ...
    DINARA_ASSERT(not kmerTable3.empty());
    ...
    )
{
    ...
    DINARA_ASSERT(alignment.size() == sequencesTable.size());
    ...
    )
{
    ...
    for(uint64_t i=1; i<sequences.size(); i++) {
        DINARA_ASSERT(sequences[i-1].second >= sequences[i].second);
    }
    ...
    // The true argument causes a final alignment entry equal to the consensus.
    vector<string> alignment = spoaAlignmentGraph.GenerateMultipleSequenceAlignment(false);
    DINARA_ASSERT(alignment.size() == sequences.size());
...
std::string dinara::globalMsaPython(
