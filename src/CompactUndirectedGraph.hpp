#ifndef DINARA_COMPACT_UNDIRECTED_GRAPH_HPP
#define DINARA_COMPACT_UNDIRECTED_GRAPH_HPP
...
template<class Vertex, class Edge>
    class dinara::CompactUndirectedGraph {
public:
    ...
    {
        DINARA_ASSERT(e.e < edgeTable.size());
        const EdgeInfo& edgeInfo = edgeTable[e.e];
        if(e.v == null_vertex()) {
            return edgeInfo.vertices[0];    // Return as stored.
        } else {
            DINARA_ASSERT(e.v==edgeInfo.vertices[0] || e.v==edgeInfo.vertices[1]);
            return e.v;
        }
    }
    vertex_descriptor target(edge_descriptor e) const
    {
        ...
        if(e.v == null_vertex()) {
            ...
            if(e.v==edgeInfo.vertices[0]) {
                ...
                DINARA_ASSERT(e.v == edgeInfo.vertices[1]);
...
namespace dinara {
    ...
    template<class Vertex, class Edge> std::pair<
        typename dinara::CompactUndirectedGraph<Vertex, Edge>::vertex_iterator,
        typename dinara::CompactUndirectedGraph<Vertex, Edge>::vertex_iterator
        > vertices(const dinara::CompactUndirectedGraph<Vertex, Edge>& graph)
    {
        return graph.allVertices();
    }

    template<class Vertex, class Edge> std::pair<
        typename dinara::CompactUndirectedGraph<Vertex, Edge>::edge_iterator,
        typename dinara::CompactUndirectedGraph<Vertex, Edge>::edge_iterator
        > edges(const dinara::CompactUndirectedGraph<Vertex, Edge>& graph)
    {
        return graph.allEdges();
    }

    template<class Vertex, class Edge>
        typename dinara::CompactUndirectedGraph<Vertex, Edge>::vertex_descriptor
        source(
        typename dinara::CompactUndirectedGraph<Vertex, Edge>::edge_descriptor e,
    ...
    template<class Vertex, class Edge> std::pair<
        typename dinara::CompactUndirectedGraph<Vertex, Edge>::out_edge_iterator,
        typename dinara::CompactUndirectedGraph<Vertex, Edge>::out_edge_iterator
        > out_edges(
            typename dinara::CompactUndirectedGraph<Vertex, Edge>::vertex_descriptor v,
            const dinara::CompactUndirectedGraph<Vertex, Edge>& graph)
    {
...
template<class Vertex, class Edge>
    inline
    dinara::CompactUndirectedGraph<Vertex, Edge>::
    CompactUndirectedGraph()
{
    clear();
}
...
template<class Vertex, class Edge>
    inline typename dinara::CompactUndirectedGraph<Vertex, Edge>::State
    dinara::CompactUndirectedGraph<Vertex, Edge>::
    getState() const
{
    return state;
...
template<class Vertex, class Edge>
    inline typename dinara::CompactUndirectedGraph<Vertex, Edge>::Int
    dinara::CompactUndirectedGraph<Vertex, Edge>::
    vertexCount() const
{
...
{
    DINARA_ASSERT(v.v < vertexTable.size());
...
template<class Vertex, class Edge>
    inline typename dinara::CompactUndirectedGraph<Vertex, Edge>::vertex_descriptor
    dinara::CompactUndirectedGraph<Vertex, Edge>::
    addVertex(const Vertex& vertex)
{
    DINARA_ASSERT(state == State::AddingVertices);
...
template<class Vertex, class Edge>
    inline typename dinara::CompactUndirectedGraph<Vertex, Edge>::edge_descriptor
    dinara::CompactUndirectedGraph<Vertex, Edge>::
    addEdge(
    vertex_descriptor v0,
    vertex_descriptor v1,
    const Edge& edge)
{
    DINARA_ASSERT(state == State::AddingEdges);
    ...
    doneAddingEdges()
{
    ...
    DINARA_ASSERT(vertexTable.front().second == Int(0));
    ...
    DINARA_ASSERT(edgeLists.size() == 2*edgeTable.size());
    DINARA_ASSERT(vertexTable.back().second == Int(edgeLists.size()));
