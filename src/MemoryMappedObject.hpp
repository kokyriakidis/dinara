...
#ifndef DINARA_MEMORY_MAPPED_OBJECT_HPP
#define DINARA_MEMORY_MAPPED_OBJECT_HPP
...
template<class T> class dinara::MemoryMapped::Object {
public:
...
// Default constructor.
template<class T> inline dinara::MemoryMapped::Object<T>::Object() :
    header(0),
    data(0),
    isOpen(false),
...
// and with write access.
// Return the file descriptor.
template<class T> inline int dinara::MemoryMapped::Object<T>::openNew(const string& name)
{
...
// Open the given existing file.
// Return the file descriptor.
template<class T> inline int dinara::MemoryMapped::Object<T>::openExisting(const string& name, bool readWriteAccess)
{
    const int fileDescriptor = ::open(
        name.c_str(),
...
// Truncate the given file descriptor to the specified size.
template<class T> inline void dinara::MemoryMapped::Object<T>::truncate(int fileDescriptor, size_t fileSize)
{
...
// Map to memory the given file descriptor for the specified size.
template<class T> inline void* dinara::MemoryMapped::Object<T>::map(int fileDescriptor, size_t fileSize, bool writeAccess)
{
...
// Find the size of the file corresponding to an open file descriptor.
template<class T> inline size_t dinara::MemoryMapped::Object<T>::getFileSize(int fileDescriptor)
{
...
template<class T> inline void dinara::MemoryMapped::Object<T>::createNew(
    const string& name,
    size_t pageSize)
{
...
// Create a new mapped object.
template<class T> inline void dinara::MemoryMapped::Object<T>::createNewAnonymous(
    size_t pageSize)
{
    try {
        // If already open, should have called close first.
...
template<class T> inline void dinara::MemoryMapped::Object<T>::accessExisting(const string& name, bool readWriteAccess)
{
    try {
        // If already open, should have called close first.
        DINARA_ASSERT(!isOpen);
...
template<class T> inline void dinara::MemoryMapped::Object<T>::accessExistingReadOnly(const string& name)
{
    accessExisting(name, false);
}
template<class T> inline void dinara::MemoryMapped::Object<T>::accessExistingReadWrite(const string& name)
{
...
// Sync the mapped memory to disk.
template<class T> inline void dinara::MemoryMapped::Object<T>::syncToDisk()
{
    DINARA_ASSERT(isOpen);
...
// Unmap the memory.
template<class T> inline void dinara::MemoryMapped::Object<T>::unmap()
{
    DINARA_ASSERT(isOpen);
...
// Sync the mapped memory to disk, then unmap it.
template<class T> inline void dinara::MemoryMapped::Object<T>::close()
{
    DINARA_ASSERT(isOpen);
...
template<class T> inline void dinara::MemoryMapped::Object<T>::remove()
{
...
// Return a pointer to the stored object.
template<class T> inline T* dinara::MemoryMapped::Object<T>::operator->()
{
    DINARA_ASSERT(isOpen);
    DINARA_ASSERT(data);
    return data;
}
template<class T> inline const T* dinara::MemoryMapped::Object<T>::operator->() const
{
    DINARA_ASSERT(isOpen);
    DINARA_ASSERT(data);
    return data;
...
// Return a reference to the stored object.
template<class T> inline T& dinara::MemoryMapped::Object<T>::object()
{
    DINARA_ASSERT(isOpen);
    DINARA_ASSERT(data);
    return *data;
}
template<class T> inline const T& dinara::MemoryMapped::Object<T>::object() const
{
    DINARA_ASSERT(isOpen);
    DINARA_ASSERT(data);
    return *data;
