...
// This class does not keep track of the number of bases
// actually stored. All unused positions are left set at "A".
template<class Int> class dinara::ShortBaseSequence {
public:
...
#if 0
        // Testing.
        DINARA_ASSERT(reverseComplementedSequence == reverseComplementSlow(n));
        DINARA_ASSERT(reverseComplementedSequence.reverseComplementSlow(n) == *this);
#endif
...
template<class Int> inline std::ostream& dinara::operator<<(
    std::ostream& s,
    const dinara::ShortBaseSequence<Int>& sequence)
{
