#ifndef DINARA_LONGEST_PATH_HPP
#define DINARA_LONGEST_PATH_HPP
...
template<class Graph> void dinara::longestPath(
    const Graph &graph,
    vector<typename Graph::vertex_descriptor>& longestPath)
{
    ...
    static_assert(
        ...
        "dinara::transitiveReduction requires an adjacency_list "
...
// Edge-based version that works with listS graphs.
// Uses assoc_property_map for topological_sort, so vertex_descriptor
// does not need to be an integer (works with boost::listS).
// This matches the interface of shasta2::longestPath.
template<class Graph> void dinara::longestPath(
    const Graph &graph,
    vector<typename Graph::edge_descriptor>& longestPath)
{
    ...
    // Map vertices to integers for topological_sort (required for listS).
    ...
    // Topological sort with explicit index map.
    vector<vertex_descriptor> sortedVertices;
    topological_sort(
        graph,
    ...
    // Find the vertex with the longest length (end of longest path).
    vertex_descriptor v = Graph::null_vertex();
    uint64_t maximumLength = 0;
    for(const auto& p: lengthMap) {
        if(p.second > maximumLength) {
    ...
    // Construct the path (edge-based), moving backward from here.
    longestPath.clear();
    while(true) {
        vertex_descriptor vPrevious = Graph::null_vertex();
...
// path lengths.
template<class Graph> void dinara::longestPath1(
    const Graph &graph,
    vector<typename Graph::vertex_descriptor>& longestPath)
{
    using namespace boost;
