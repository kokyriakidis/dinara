#ifndef DINARA_COMPUTE_SFDP_LAYOUT
#define DINARA_COMPUTE_SFDP_LAYOUT
...
user and is not part of Dinara. It is invoked as follows:
...
template<class Graph> dinara::ComputeLayoutReturnCode dinara::computeLayoutGraphviz(
    const Graph& graph,
    const string& layoutMethod,
    double timeout,
    std::map<typename Graph::vertex_descriptor, array<double, 2> >& positionMap,
...
template<class Graph> dinara::ComputeLayoutReturnCode dinara::computeLayoutCustom(
    const Graph& graph,
    const std::map<typename Graph::edge_descriptor, double>& edgeLengthMap,
    std::map<typename Graph::vertex_descriptor, array<double, 2> >& positionMap,
    uint64_t quality,
