...
// A Chain is a sequence of AnchorIds.
class dinara::mode3::Chain : public vector<AnchorId> {
public:
...
class dinara::mode3::Bubble : public vector<Chain> {
public:
...
class dinara::mode3::BubbleChain : public vector<Bubble> {
public:
    const Bubble& firstBubble() const
    {
        DINARA_ASSERT(not empty());
        return front();
    }
    Bubble& firstBubble()
    {
    ...
    AnchorId firstAnchorId() const
    {
        ...
        for(const Chain& chain: firstBubble) {
            DINARA_ASSERT(chain.front() == anchorId);
        }
    ...
    AnchorId lastAnchorId() const
    {
        ...
        for(const Chain& chain: lastBubble) {
            DINARA_ASSERT(chain.back() == anchorId);
        }
...
class dinara::mode3::ChainIdentifier {
public:
...
class dinara::mode3::AssemblyGraphVertex {
public:
...
class dinara::mode3::AssemblyGraphEdge : public BubbleChain {
public:
...
class dinara::mode3::AssemblyGraph:
    public AssemblyGraphBaseClass,
    public MultithreadedObject<dinara::mode3::AssemblyGraph>,
...
// true or false.
class dinara::mode3::AssemblyGraphEdgePredicate {
public:
...
// out-degree(v0) > 1
// in_degree(v1) > 1.
class dinara::mode3::AssemblyGraphCrossEdgePredicate : public AssemblyGraphEdgePredicate {
public:
...
class dinara::mode3::AssemblyGraphNoInternalAnchorsEdgePredicate : public AssemblyGraphEdgePredicate {
public:
    bool operator()(edge_descriptor) const;
