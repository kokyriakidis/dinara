...
// New constructor from the global Shasta2AnchorGraph.
// vertex and edge descriptors use a L (local) suffix for the Shasta2LocalAnchorGraph
// and G (global) for the Shasta2AnchorGraph.
Shasta2LocalAnchorGraph::Shasta2LocalAnchorGraph(
    const Shasta2Anchors& anchors,
    const Shasta2AnchorGraph& anchorGraph,
    const vector<Shasta2AnchorId>& anchorIds,
...
{
    Shasta2LocalAnchorGraph& localAnchorGraph = *this;

    // Initialize a BFS from these AnchorIds.
    std::queue<vertex_descriptor> q;
    for(const Shasta2AnchorId anchorId: anchorIds) {
        DINARA_ASSERT(not vertexMap.contains(anchorId));
        const vertex_descriptor vL = boost::add_vertex(Shasta2LocalAnchorGraphVertex(anchorId, 0), localAnchorGraph);
    ...
    while(not q.empty()) {
        ...
        q.pop();
        const Shasta2LocalAnchorGraphVertex& vertex0L = localAnchorGraph[v0L];
        const Shasta2AnchorId anchorId0 = vertex0L.anchorId;

        const uint64_t distance0 = vertex0L.distance;
        const uint64_t distance1 = distance0 + 1;

        // Get the corresponding global Shasta2AnchorGraph vertex.
        // In the global Shasta2AnchorGraph, vertex descriptors are AnchorIds.
        const Shasta2AnchorGraph::vertex_descriptor v0G = anchorId0;

        // Loop over out-edges.
        BGL_FORALL_OUTEDGES(v0G, eG, anchorGraph, Shasta2AnchorGraph) {
            ...
            const Shasta2AnchorId anchorId1 = v1G;
            ...
            const vertex_descriptor v1L = boost::add_vertex(Shasta2LocalAnchorGraphVertex(anchorId1, distance1), localAnchorGraph);
        ...
        // Loop over in-edges.
        BGL_FORALL_INEDGES(v0G, eG, anchorGraph, Shasta2AnchorGraph) {
            if(edgesMarkedForAssembly and (not anchorGraph[eG].useForAssembly)) {
                continue;
            }
            if(anchorGraph[eG].coverage() < minCoverage) {
    ...
    // Now add the edges.
    BGL_FORALL_VERTICES(v0L, localAnchorGraph, Shasta2LocalAnchorGraph) {
        const Shasta2LocalAnchorGraphVertex& vertex0L = localAnchorGraph[v0L];
        const Shasta2AnchorId anchorId0 = vertex0L.anchorId;
        ...
        BGL_FORALL_OUTEDGES(v0G, eG, anchorGraph, Shasta2AnchorGraph) {
            ...
            if(it1 != vertexMap.end()) {
                ...
                Shasta2LocalAnchorGraphEdge edge;
                edge.eG = eG;
    ...
    const Shasta2AssemblyGraphPostprocessor* assemblyGraph3Pointer) const
{
    ...
    if(options.vertexColoring == "byReadComposition") {
        referenceAnchorId = shasta2AnchorIdFromString(options.referenceAnchorIdString);
        if((referenceAnchorId == invalid<Shasta2AnchorId>) or (referenceAnchorId >= anchors.size())) {
    ...
    s << "digraph Shasta2LocalAnchorGraph {\n";

    // Write the vertices.
    BGL_FORALL_VERTICES(v, graph, Shasta2LocalAnchorGraph) {
        const Shasta2LocalAnchorGraphVertex& vertex = graph[v];
        const Shasta2AnchorId anchorId = vertex.anchorId;
    ...
    // Write the edges.
    BGL_FORALL_EDGES(e, graph, Shasta2LocalAnchorGraph) {
        const Shasta2LocalAnchorGraphEdge& edge = graph[e];

        const vertex_descriptor v0 = source(e, graph);
        const vertex_descriptor v1 = target(e, graph);

        const Shasta2LocalAnchorGraphVertex& vertex0 = graph[v0];
        const Shasta2LocalAnchorGraphVertex& vertex1 = graph[v1];

        const Shasta2AnchorId anchorId0 = vertex0.anchorId;
        const Shasta2AnchorId anchorId1 = vertex1.anchorId;

        const string anchorId0String = shasta2AnchorIdToString(anchorId0);
        const string anchorId1String = shasta2AnchorIdToString(anchorId1);
...
Shasta2LocalAnchorGraphDisplayOptions::Shasta2LocalAnchorGraphDisplayOptions(const vector<string>& request)
{
...
void Shasta2LocalAnchorGraphDisplayOptions::writeForm(ostream& html) const
{
    // Figure out if command "customLayout" is available.
    const int commandStatus = std::system("which customLayout > /dev/null");
    DINARA_ASSERT(WIFEXITED(commandStatus));
    ...
    const Shasta2AssemblyGraphPostprocessor* assemblyGraph3Pointer)
{
    ...
    // Begin the svg.
    const string svgId = "Shasta2LocalAnchorGraph";
    html <<
...
void Shasta2LocalAnchorGraph::computeLayout(const Shasta2LocalAnchorGraphDisplayOptions& options)
{
    const Shasta2LocalAnchorGraph& graph = *this;
    DINARA_ASSERT(anchorGraphPointer);
    const Shasta2AnchorGraph& anchorGraph = *anchorGraphPointer;
...
void Shasta2LocalAnchorGraph::computeLayoutBoundingBox()
{
...
void Shasta2LocalAnchorGraph::Box::makeSquare()
{
    if(xSize() > ySize()) {
...
void Shasta2LocalAnchorGraph::Box::extend(double factor)
{
...
void Shasta2LocalAnchorGraph::writeEdges(
    ostream& html,
    const Shasta2LocalAnchorGraphDisplayOptions& options) const
{
    ...
    BGL_FORALL_EDGES(eL, graph, Shasta2LocalAnchorGraph) {
        ...
        const Shasta2LocalAnchorGraphVertex& vertex0 = graph[v0];
        const Shasta2AnchorId anchorId0 = vertex0.anchorId;
        const string anchorIdString0 = shasta2AnchorIdToString(anchorId0);
        const Shasta2LocalAnchorGraphVertex& vertex1 = graph[v1];
        const Shasta2AnchorId anchorId1 = vertex1.anchorId;
        const string anchorIdString1 = shasta2AnchorIdToString(anchorId1);
...
void Shasta2LocalAnchorGraph::writeSvgControls(
    ostream& html,
    const Shasta2LocalAnchorGraphDisplayOptions& /* options */) const
{
    html <<
