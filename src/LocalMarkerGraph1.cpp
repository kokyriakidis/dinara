    ...
    uint64_t distance)
{
    ...
    DINARA_ASSERT(not vertexMap.contains(vertexId));
    ...
    bool useSvg) const
{
    ...
    if(useSvg) {
        ...
        for(const auto& pixel :pixels) {
            ...
            DINARA_ASSERT(ix >= 0);
            DINARA_ASSERT(ix < width);
            const int64_t iy = ixy.second - iyMin;
            DINARA_ASSERT(iy >= 0);
            DINARA_ASSERT(iy < height);
    ...
    double timeout) const
{
    ...
    if(coloring == "readFollowing") {
        ...
        for(const MarkerInterval& startMarkerInterval:
            ...
            for(int64_t offset=firstMarkerOffset; offset<=lastMarkerOffset; offset++) {
                ...
                DINARA_ASSERT(compressedVertexId0 != MarkerGraph::invalidCompressedVertexId);
                DINARA_ASSERT(compressedVertexId1 != MarkerGraph::invalidCompressedVertexId);
                ...
                DINARA_ASSERT(edgeId != invalid<MarkerGraphEdgeId>);
    ...
    for(const auto& p: allEdges) {
        ...
        DINARA_ASSERT(not auxiliaryVertices.empty());
        ...
        html <<
            ...
            ", " << sequence.size() << " bases: ";
        copy(sequence.begin(), sequence.end(), ostream_iterator<dinara::Base>(html));
        if(coloring == "readFollowing") {
