    ...
    detangleOffDiagonalRatio(detangleOffDiagonalRatio)
{
    ...
    BGL_FORALL_VERTICES(v, graph, AssemblyPathGraph2) {
        DINARA_ASSERT(graph[graph[v].reverseComplementVertex].reverseComplementVertex == v);
    }
    ...
    BGL_FORALL_EDGES(e, graph, AssemblyPathGraph2) {
        DINARA_ASSERT(graph[graph[e].reverseComplementEdge].reverseComplementEdge == e);
    }
}
...
void AssemblyPathGraph2::writeGraphviz(ostream& s) const
{
    ...
    BGL_FORALL_EDGES(e, graph, AssemblyPathGraph2) {
        ...
        if(edge.tangle != invalidTangle2Id) {
            // Tangle2 edge.
            DINARA_ASSERT(edge.inTangle == invalidTangle2Id);
            DINARA_ASSERT(edge.outTangle == invalidTangle2Id);
...
bool AssemblyPathGraph2::createTangleAtEdge(edge_descriptor e01)
{
    ...
    Tangle2 tangle;
    tangle.edge = e01;
    DINARA_ASSERT(graph[e01].tangle == invalidTangle2Id);
...
Tangle2& AssemblyPathGraph2::getTangle(Tangle2Id tangleId)
{
    ...
    DINARA_ASSERT(it != tangles.end());
    Tangle2& tangle = it->second;
    DINARA_ASSERT(tangle.tangleId == tangleId);
    return tangle;
}
    ...
    Tangle2Id tangleId) const
{
    ...
    DINARA_ASSERT(reverseComplementTangleId != invalidTangle2Id);
    ...
    const AssemblyGraph& assemblyGraph)
{
    ...
    for(const edge_descriptor e: newEdges)  {
        ...
        DINARA_ASSERT(wasFound);
    }

    // Sanity check.
    for(const edge_descriptor e: newEdges)  {
        const  edge_descriptor eRc = graph[e].reverseComplementEdge;
        DINARA_ASSERT(graph[eRc].reverseComplementEdge == e);
    }
}
    ...
    vector<edge_descriptor>& newEdges)
{
    ...
    DINARA_ASSERT(tangle.isSolvable);
    ...
    for(const edge_descriptor e: tangle.inEdges) {
        const AssemblyPathGraph2Edge& edge = graph[e];
        DINARA_ASSERT(edge.outTangle == tangleId);
        DINARA_ASSERT(edge.tangle == invalidTangle2Id);
    ...
    for(const edge_descriptor e: tangle.outEdges) {
        const AssemblyPathGraph2Edge& edge = graph[e];
        DINARA_ASSERT(edge.tangle == invalidTangle2Id);
        DINARA_ASSERT(edge.inTangle == tangleId);
    ...
    DINARA_ASSERT(in_degree(v0, graph) == 0);
    DINARA_ASSERT(out_degree(v0, graph) == 0);
    DINARA_ASSERT(in_degree(v1, graph) == 0);
    DINARA_ASSERT(out_degree(v1, graph) == 0);
    ...
    vector<edge_descriptor>& newEdges)
{
    ...
    */
    DINARA_ASSERT(tangleA.isSolvable);
    DINARA_ASSERT(tangleB.isSolvable);
    AssemblyPathGraph2& graph = *this;
    ...
    for(uint64_t i=0; i<tangle0.inEdges.size(); i++) {
        ...
        DINARA_ASSERT(it != tangle1.inEdges.end());
    ...
    for(const edge_descriptor e: tangle0.inEdges) {
        const AssemblyPathGraph2Edge& edge = graph[e];
        DINARA_ASSERT(edge.outTangle == tangleId0);
        DINARA_ASSERT(edge.tangle == invalidTangle2Id);
        if(edge.inTangle != invalidTangle2Id) {
            tanglesToBeRemoved.push_back(edge.inTangle);
            /*
    ...
    for(const edge_descriptor e: tangle1.outEdges) {
        const AssemblyPathGraph2Edge& edge = graph[e];
        DINARA_ASSERT(edge.tangle == invalidTangle2Id);
        DINARA_ASSERT(edge.inTangle == tangleId1);
        if(edge.outTangle != invalidTangle2Id) {
            tanglesToBeRemoved.push_back(edge.outTangle);
            /*
            cout << "Will remove following tangle " << edge.outTangle <<
    ...
    for(vertex_descriptor v: verticesToBeRemoved) {
        DINARA_ASSERT(in_degree(v, graph) == 0);
        DINARA_ASSERT(out_degree(v, graph) == 0);
