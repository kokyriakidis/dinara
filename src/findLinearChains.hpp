#ifndef DINARA_FIND_LINEAR_CHAINS_HPP
#define DINARA_FIND_LINEAR_CHAINS_HPP
...
template<class Graph> void dinara::findLinearChains(
    const Graph& graph,
    uint64_t minimumLength,
...
// Find linear chains of edges (paths).
template<class Graph> inline void dinara::findLinearChains(
    const Graph& graph,
    uint64_t minimumLength,
    vector< std::list<typename Graph::edge_descriptor> >& chains)
{
    ...
    // DINARA_ASSERT(edgesFound.size() == num_edges(graph));
    uint64_t edgeCount = 0;
    BGL_FORALL_EDGES_T(e, graph, Graph) {
        ++edgeCount;
    }
    DINARA_ASSERT(edgesFound.size() == edgeCount);
}



// Find linear chains of vertices.
template<class Graph> void dinara::findLinearVertexChains(
    const Graph& graph,
    vector< std::list<typename Graph::vertex_descriptor> >& chains)
{
    ...
    BGL_FORALL_VERTICES_T(vStart, graph, Graph) {
        ...
        while(true) {
            ...
            chain.push_back(v);
            DINARA_ASSERT(verticesFound.find(v) == verticesFound.end());
    ...
    // DINARA_ASSERT(verticesFound.size() == num_vertices(graph));
    ...
    DINARA_ASSERT(verticesFound.size() == vertexCount);
}
