    ...
    size_t maxCoverage)
{
    ...
    for(MarkerGraph::VertexId vertexId=0;
        vertexId!=markerGraph.vertexCount(); vertexId++) {
        const auto markers = markerGraph.getVertexMarkerIds(vertexId);
        DINARA_ASSERT(markers.size() >= minCoverage);
        DINARA_ASSERT(markers.size() <= maxCoverage);
        for(const MarkerId markerId: markers) {
            if(markerGraph.vertexTable[markerId] != vertexId) {
                cout << "Failure at vertex " << vertexId << " marker " << markerId << endl;
            }
            DINARA_ASSERT(markerGraph.vertexTable[markerId] == vertexId);
        }
    }
}
...
void Assembler::createMarkerGraphVerticesThreadFunction4(uint64_t threadId)
{
    ...
    while(getNextBatch(begin, end)) {
        for(MarkerId i=begin; i!=end; ++i) {
            const MarkerId rawVertexId = markerGraph.vertexTable[i];
            DINARA_ASSERT(rawVertexId != maxValueMinus1);
    ...
    while(getNextBatch(begin, end)) {
        for(EdgeId edgeId=begin; edgeId!=end; edgeId++) {
            ...
            for(const Uint40 edgeIdRc: v1rcOutEdges) {
                ...
                DINARA_ASSERT(edgeRc.source == v1Rc);
...
void Assembler::checkMarkerGraphIsStrandSymmetricThreadFunction1(uint64_t)
{
    ...
    while(getNextBatch(begin, end)) {
        ...
        for (MarkerGraph::VertexId v0=begin; v0!=end; v0++) {
            ...
            DINARA_ASSERT(markers0.size() == markers1.size());
            ...
            for (size_t i = 0; i < markers0.size(); i++) {
                ...
                DINARA_ASSERT(markerGraph.vertexTable[markerId0] == v0);
                DINARA_ASSERT(markerGraph.vertexTable[markerId1] == v1);
            }
        }
    }
}
...
void Assembler::checkMarkerGraphIsStrandSymmetricThreadFunction2(uint64_t)
{
    ...
    while(getNextBatch(begin, end)) {
        for (EdgeId e0=begin; e0!=end; e0++) {
            ...
            DINARA_ASSERT(markerGraph.edgeCoverage(e0) == markerGraph.edgeCoverage(e1));
            DINARA_ASSERT(
                edge0.wasRemovedByTransitiveReduction
                == edge1.wasRemovedByTransitiveReduction);
            DINARA_ASSERT(edge0.wasPruned == edge1.wasPruned);
            DINARA_ASSERT(edge0.isSuperBubbleEdge == edge1.isSuperBubbleEdge);
            DINARA_ASSERT(edge0.isLowCoverageCrossEdge == edge1.isLowCoverageCrossEdge);
            ...
            DINARA_ASSERT(markerIntervals0.size() == markerIntervals1.size());
...
void Assembler::writeBadMarkerGraphVertices() const
{
    ...
    for(MarkerGraph::VertexId vertexId=0; vertexId!=markerGraph.vertexCount(); vertexId++) {
        ...
        DINARA_ASSERT(markerIds.size() > 0);
    ...
    )
{
    ...
    while(!q.empty()) {
        ...
        for(uint64_t edgeId: childEdges) {
            ...
            DINARA_ASSERT(edge.source == vertexId0);
            DINARA_ASSERT(vertexId1 < markerGraph.vertexCount());
        ...
        for(uint64_t edgeId: parentEdges) {
            ...
            DINARA_ASSERT(edge.target == vertexId0);
            DINARA_ASSERT(vertexId1 < markerGraph.vertexCount());
...
void Assembler::createMarkerGraphEdges(size_t threadCount)
{
    ...
    for(size_t threadId=0; threadId<threadCount; threadId++) {
        ...
        DINARA_ASSERT(thisThreadEdges.size() == thisThreadEdgeMarkerIntervals.size());
    ...
    DINARA_ASSERT(markerGraph.edges.size() == markerGraph.edgeMarkerIntervals.size());
...
{
    DINARA_ASSERT(markerGraph.edges.isOpen);
    ...
    size_t edgeMarkerSkipThreshold)
{
    ...
    DINARA_ASSERT(edgesByCoverage[0].size() == 0);
    ...
    ) const
{
    ...
    for(size_t i=0; i<2; i++) {
        DINARA_ASSERT(verticesByDistance[i].size() == maxDistance+1);
        DINARA_ASSERT(vertexFlags[i].size() == markerGraph.vertexCount());
    }
    ...
    for(size_t i=0; i<2; i++) {
        DINARA_ASSERT(verticesByDistance[i][0].size() == 0);
    ...
    for(size_t distance=0; distance<maxDistance; distance++) {
        ...
        for(size_t i=0; i<2; i++) {
            ...
            DINARA_ASSERT(verticesByDistance[i][distance+1].size() == 0);
    ...
    MarkerGraph::EdgeId edgeId0) const
{
    ...
    DINARA_ASSERT(!edge0.wasRemoved());
    ...
    )
{
    ...
    DINARA_ASSERT(mode1Count + mode2Count == markerCount);
    ...
    DINARA_ASSERT(mode2Count > mode1Count);
    ...
    for(size_t position=0; position<alignmentLength; position++) {
        ...
        if(!consensus.base.isGap()) {
            ...
            DINARA_ASSERT(consensus.repeatCount > 0);
    ...
    bool debug)
{
    ...
    for (AssemblyGraph::VertexId componentId = 0; componentId < n; componentId++) {
        ...
        DINARA_ASSERT(rcComponentTable[componentRcId] == componentId);
    ...
    for (AssemblyGraph::VertexId v0 = 0; v0 < n; v0++) {
        ...
        DINARA_ASSERT(rcComponentTable[c0] == c1);
        DINARA_ASSERT(rcComponentTable[c1] == c0);
    }
    ...
    for(AssemblyGraph::VertexId componentId=0; componentId<n; componentId++) {
        ...
        // Check for that.
        DINARA_ASSERT(std::is_sorted(component.begin(), component.end()));
        ...
        for(uint64_t entryIndex=0; entryIndex<component.size(); entryIndex++) {
            ...
            for(uint64_t exitIndex=0; exitIndex<component.size(); exitIndex++) {
                ...
                while(true) {
                    ...
                    DINARA_ASSERT(e != AssemblyGraph::invalidEdgeId);
...
void Assembler::assembleMarkerGraphVertices(size_t threadCount)
{
    ...
    DINARA_ASSERT(assemblerInfo->readRepresentation == 1);
...
void Assembler::assembleMarkerGraphVerticesThreadFunction(uint64_t)
{
    ...
    while(getNextBatch(begin, end)) {
        ...
        for(MarkerGraph::VertexId vertexId=begin; vertexId!=end; vertexId++) {
            ...
            // Store them.
            DINARA_ASSERT(repeatCounts.size() == k);
...
void Assembler::computeMarkerGraphVerticesCoverageData(size_t threadCount)
{
    ...
    for(MarkerGraph::VertexId vertexId=0; vertexId!=markerGraph.vertexCount(); vertexId++) {
        ...
        DINARA_ASSERT(threadId != invalidValue);
        DINARA_ASSERT(i != invalidValue);
...
void Assembler::accessMarkerGraphCoverageData()
{
    try {
        ...
        throw runtime_error("Coverage data is not available. It is only stored if dinara.conf has "
...
void Assembler::assembleMarkerGraphEdgesThreadFunction(size_t threadId)
{
    ...
    while(getNextBatch(begin, end)) {
        ...
        for(MarkerGraph::EdgeId edgeId=begin; edgeId!=end; edgeId++) {
            ...
            DINARA_ASSERT(repeatCounts.size() == n);
