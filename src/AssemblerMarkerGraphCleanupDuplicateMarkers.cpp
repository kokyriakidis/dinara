...
These vertices, sometimes referred to in Dinara code and documentation as
    ...
    bool pattern2CreateNewVertices)
{
    ...
    if(debug) {
        for(MarkerGraph::VertexId vertexId=0; vertexId<markerGraph.vertices().size(); vertexId++) {
            ...
            DINARA_ASSERT(markerGraph.vertices().size(vertexId) > 0);
        }
    }
...
void Assembler::cleanupDuplicateMarkersThreadFunction(size_t threadId)
{
    ...
    while(getNextBatch(begin, end)) {
        ...
        for(MarkerGraph::VertexId vertexId=begin; vertexId!=end; ++vertexId) {
            ...
            DINARA_ASSERT(duplicateCount > 0);
            ...
            if(duplicateRatio < pattern1Threshold) {
                ...
                DINARA_ASSERT(duplicateCount < markerCount);
                if(vertexId == vertexIdRc) {
    ...
    ostream& out)
{
    ...
    DINARA_ASSERT(isDuplicateOrientedReadId.size() == markerCount);
    ...
    ostream& out)
{
    ...
    DINARA_ASSERT(num_vertices(graph) == markerCount);
