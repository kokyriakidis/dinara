...
int64_t dinara::Histogram2::findIndex(double x){
...
void dinara::Histogram2::update(double x) {
...
void dinara::Histogram2::getNormalizedHistogram(vector<double>& normalizedHistogram){
    uint64_t sum = getSum();
...
pair<string,string> dinara::Histogram2::getBoundStrings(uint64_t binIndex, int32_t precision){
...
void dinara::Histogram2::writeToHtml(ostream& html, uint64_t sizePx, int32_t precision){
    uint64_t yMax = 0;
    for (auto& e: histogram){
        if (e > yMax){
            yMax = e;
...
void dinara::writeHistogramsToHtml(
        ostream& html,
...
void dinara::Histogram2::writeToCsv(ostream& csv, int32_t precision){
...
void dinara::testIterativeHistogram(){
