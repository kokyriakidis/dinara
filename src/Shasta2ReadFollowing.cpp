...
using namespace Shasta2ReadFollowing;
...
Graph::Graph(const Shasta2AssemblyGraph& assemblyGraph) :
...
// Create vertices of the Shasta2ReadFollowing graph.
// Each vertex corresponds to a Segment of the Shasta2AssemblyGraph.
void Graph::createVertices()
{
    ...
    BGL_FORALL_EDGES(segment, assemblyGraph, Shasta2AssemblyGraph) {
...
Vertex::Vertex(
    ...
    segment(segment)
{
    const Shasta2AssemblyGraphEdge& edge = assemblyGraph[segment];
    ...
    for(const auto& step: edge) {
        const double weight = max<uint64_t>(1, step.offset);
        weightedCoverage += weight * double(step.anchorPair.size());
        totalWeight += weight;
    }
    coverage = (totalWeight > 0.) ? (weightedCoverage / totalWeight) : 0.;

    // Compute initial/final support.
    const uint32_t representativeRegionStepCount =  uint32_t(assemblyGraph.getOptions().representativeRegionStepCount);
    Shasta2SegmentStepSupport::getInitialFirst(assemblyGraph, segment, representativeRegionStepCount, initialSupport);
    Shasta2SegmentStepSupport::getFinalLast   (assemblyGraph, segment, representativeRegionStepCount, finalSupport  );
}



// Create edges of the Shasta2ReadFollowing graph.
// An edge v0->v1 is created if the final support of v0
// shares at least one OrientedReadId with the initial support of v1.
void Graph::createEdges()
{
    Graph& graph = *this;
    const Shasta2Journeys* journeys = assemblyGraph.getJourneysPointer();
    DINARA_ASSERT(journeys != nullptr);
    const uint64_t orientedReadCount = journeys->size();
    const uint64_t minCommonCount = assemblyGraph.getOptions().readFollowingMinCommonCount;
    const double minCorrectedJaccard = assemblyGraph.getOptions().readFollowingMinCorrectedJaccard;
    ...
    BGL_FORALL_VERTICES(v, graph, Graph) {
        ...
        for(const Shasta2SegmentStepSupport& s: vertex.initialSupport) {
            initialSupportVertices[s.orientedReadId.getValue()].push_back(v);
        }

        for(const Shasta2SegmentStepSupport& s: vertex.finalSupport) {
    ...
    DINARA_ASSERT(vertexPairs.size() == count.size());
    ...
    for(uint64_t i=0; i<vertexPairs.size(); i++) {
        ...
        DINARA_ASSERT(edge.segmentPairInformation.commonCount >= minCommonCount);
        DINARA_ASSERT(edge.segmentPairInformation.commonCount == count[i]);
    ...
    Segment segment1)
{
    const uint32_t representativeRegionStepCount =  uint32_t(assemblyGraph.getOptions().representativeRegionStepCount);
    ostream html(0);
    segmentPairInformation = Shasta2SegmentStepSupport::analyzeSegmentPair(
        html, assemblyGraph, segment0, segment1, representativeRegionStepCount);

    DINARA_ASSERT(segmentPairInformation.commonCount > 0);
}
...
{
    cout << "Shasta2ReadFollowing-" << name << ": " << num_vertices(*this) <<
...
void Graph::writeGraphviz(const string& name) const
{
    ...
    ofstream dot("Shasta2ReadFollowing-" + name + ".dot");
    dot << "digraph Shasta2ReadFollowing {\n";
    dot << std::fixed << std::setprecision(1);

    BGL_FORALL_VERTICES(v, graph, Graph) {
        const Vertex& vertex = graph[v];
        const Segment segment = vertex.segment;
        const Shasta2AssemblyGraphEdge& assemblyGraphEdge = assemblyGraph[segment];
        ...
        // Color.
        if(vertex.length >= assemblyGraph.getOptions().readFollowingSegmentLengthThreshold) {
    ...
    BGL_FORALL_EDGES(e, graph, Graph) {
        ...
        // Green = 1
        // Red = assemblyGraph.getOptions().readFollowingMinCorrectedJaccard.
        double hue;
        if(edge.segmentPairInformation.correctedJaccard >= 1.) {
            hue = 1.;
        } else if(edge.segmentPairInformation.correctedJaccard <= assemblyGraph.getOptions().readFollowingMinCorrectedJaccard) {
            hue = 0.;
        } else {
            hue =
                (edge.segmentPairInformation.correctedJaccard - assemblyGraph.getOptions().readFollowingMinCorrectedJaccard) /
                (1. - assemblyGraph.getOptions().readFollowingMinCorrectedJaccard);
        }
        hue /= 3.;
...
void Graph::writeVerticesCsv(const string& name) const
{
    ...
    ofstream csv("Shasta2ReadFollowing-Vertices-" + name + ".csv");
...
void Graph::writeEdgesCsv(const string& name) const
{
    ...
    ofstream csv("Shasta2ReadFollowing-Edges-" + name + ".csv");
...
bool Graph::pruneIteration()
{
    ...
    BGL_FORALL_VERTICES(v, graph, Graph) {
        if(graph[v].length < assemblyGraph.getOptions().readFollowingPruneLength) {
...
void Graph::findPaths([[maybe_unused]] vector< vector<Segment> >& assemblyPaths)
{
    ...
    BGL_FORALL_VERTICES(u0, pathGraph, PathGraph) {
        ...
        DINARA_ASSERT(it0 != vertexMap.end());
    ...
    for(const vector<PathGraph::vertex_descriptor>& chain: chains) {
        ...
        for(uint64_t i1=1; i1<chain.size(); i1++) {
            ...
            DINARA_ASSERT(pathGraph[u0].segment == pathSegments.front());
            DINARA_ASSERT(pathGraph[u1].segment == pathSegments.back());
...
#if 0
// Find assembly paths.
// Note these are paths in the Shasta2ReadFollowing::Graph but not in the Shasta2AssemblyGraph.
void Graph::findPaths(vector< vector<Segment> >& assemblyPaths) const
{
    ...
    for(const vector<PathGraph::vertex_descriptor>& chain: chains) {
        ...
        for(uint64_t i1=1; i1<chain.size(); i1++) {
            ...
            DINARA_ASSERT(pathGraph[u0].segment == path.front());
            DINARA_ASSERT(pathGraph[u1].segment == path.back());
...
PathGraph::PathGraph(const Shasta2AssemblyGraph& assemblyGraph) :
    assemblyGraph(assemblyGraph)
{}
...
{
    const PathGraph& pathGraph = *this;
    DINARA_ASSERT(in_degree(v, pathGraph) > 0);
...
{
    const PathGraph& pathGraph = *this;
    DINARA_ASSERT(out_degree(v, pathGraph) > 0);
...
void dinara::Shasta2ReadFollowingGraph::findPaths(
    ...
    vector< vector<Shasta2AssemblyGraph::edge_descriptor> >& assemblyPaths) const
{
    ...
    Shasta2ReadFollowing::Graph graph(assemblyGraph);
    vector< vector<Shasta2AssemblyGraph::edge_descriptor> > allPaths;
    ...
    for(const auto& path: allPaths) {
        ...
        assemblyPaths.push_back(path);
        if(maxPathCount != 0 && assemblyPaths.size() >= maxPathCount) {
            break;
        }
    }
}
